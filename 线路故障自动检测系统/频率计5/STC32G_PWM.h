...
#define PWMA_UIEA_Enable()		PWMA_IER |= 0x01		//1：允许更新事件中断（如：计数器上溢，下溢及初始化）
...
#define CCMRn_FREEZE				0x00	//冻结	PWMn_CCR1与PWMn_CNT间的比较对OCnREF不起作用
#define CCMRn_MATCH_VALID			0x10	//匹配时设置通道 n 的输出为有效电平	当PWMn_CCR1=PWMn_CNT时，OCnREF输出高
#define CCMRn_MATCH_INVALID			0x20	//匹配时设置通道 n 的输出为无效电平	当PWMn_CCR1=PWMn_CNT时，OCnREF输出低
#define CCMRn_ROLLOVER				0x30	//翻转	当PWMn_CCR1=PWMn_CNT时，翻转OCnREF
#define CCMRn_FORCE_INVALID			0x40	//强制为无效电平	强制OCnREF为低
#define CCMRn_FORCE_VALID			0x50	//强制为有效电平	强制OCnREF为高
#define CCMRn_PWM_MODE1				0x60	//PWM 模式 1	在向上计数时，当PWMn_CNT<PWMn_CCR1时 OCnREF输出高，否则OCnREF输出低 在向下计数时，当PWMn_CNT>PWMn_CCR1时 OCnREF输出低，否则OCnREF输出高
#define CCMRn_PWM_MODE2				0x70	//PWM 模式 2	在向上计数时，当PWMn_CNT<PWMn_CCR1时 OCnREF输出低，否则OCnREF输出高 在向下计数时，当PWMn_CNT>PWMn_CCR1时 OCnREF输出高，否则OCnREF输出低
//只有PWM1P/PWM2P/PWM3P/PWM4P 才有捕获功能和测量脉宽功能
...
void HPWM_Set(u8 PWMchannel,float zkb);	//以百分比0~1形式调整占空比
